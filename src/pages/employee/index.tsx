...
const Employee = () => {
  ...
  const filteredEmployees = employees.filter(emp => {
    const matchesSearch = `${emp.firstName} ${emp.lastName} ${emp.id} ${emp.email}`
      .toLowerCase()
  ...
  return (
    <div className="p-10 space-y-8 bg-[#fcfdfc] min-h-screen">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-xl font-black text-slate-900 tracking-tight ">Employees</h1>
          <p className="text-slate-400 text-sm mt-1">All our employees list can be found here.</p>
        </div>
